...
In this article, I will outline the steps to deploy [Cloudera Streams Messaging Operator](https://docs.cloudera.com/csm-operator/1.2/index.html), using Strimzi on Kubernetes platform, and demonstrate how to integrate the Kafka cluster with existing Prometheus and Grafana platforms. The connectivity between the components mentioned above is shown in the architecture diagram below.
...
11. The final step is to import the [Grafana template for Strimzi](https://github.com/strimzi/strimzi-kafka-operator/tree/0.43.0/examples/metrics/grafana-dashboards) and view the newly created dashboard.
